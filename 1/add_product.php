         ...
         <header id="header" class="header">
            <div class="top-left">
               <div class="navbar-header">
                  <a class="navbar-brand" href="index.php">DropMate</a>
                  <a class="navbar-brand hidden" href="index.php"><img src="./admin/images/logo2.png" alt="Logo"></a>
...
//             $destinationPath = 'C:/xampp/htdocs/1/1/uploads/image.jpg';
//             $compressionQuality = 75; // Example quality value
            
//             // Now you can call your image compression function
//             imageCompress($tempFilePath, $destinationPath, $compressionQuality);
            
//             // Further processing, like updating the database, etc.
//         } else {
...
// // Other parts of your manage_product.php script

// Handle form submission
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    foreach ($_POST['mrp'] as $index => $mrp) {
        $price = $_POST['price'][$index];
        $qty = $_POST['qty'][$index];
        
        // Insert or update the database with $mrp, $price, and $qty
        $sql = "INSERT INTO product (mrp, price, qty) VALUES ('$mrp', '$price', '$qty')";
        mysqli_query($con, $sql);
    }
    // You can also include a redirect or a success message after the operation
}
			...
			if(isset($_FILES['product_images']['name'])){
				...
				if($_FILES['product_images']['name'][$key]!=''){
					if(isset($_POST['product_images_id'][$key])){
						...
						//move_uploaded_file($_FILES['product_images']['tmp_name'][$key],PRODUCT_MULTIPLE_IMAGE_SERVER_PATH.$image);
						// imageCompress($_FILES['product_images']['tmp_name'][$key],PRODUCT_MULTIPLE_IMAGE_SERVER_PATH.$image);
						 imageCompress('C:\\xampp\\tmp\\php123.tmp', 'C:/xampp/htdocs/1/1/uploads/image.jpg',75);
						//imageCompress('C:\\xampp\\tmp\\php...', 'C:/xampp/htdocs...', 75);  // Assuming '75' is the quality or third parameter
	...
	echo '<input type="hidden" name="product_images_id[]" value="'.$list['id'].'"/></div>';
		 ...
		 <script>
			...
			function remove_attr(attr_count,id){
				jQuery.ajax({
					url:'./admin/remove_product_attr.php',
					data:'id='+id,
					type:'post',
					success:function(result){
